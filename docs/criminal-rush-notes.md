...
- **Mutants / Robot Soldier / Cannon / Barbed Wire / Hammer / Pistol / Mosquitos stacking**: resolution order — (1) Mutants chooses which weapon to copy first; (2) resolve "before combat" effects for attacker then defender (Hammer, Mosquitos, etc.); (3) resolve PL-scaling effects (Robot Soldier, Cannon) after. Robot Soldier copied via Mutants still caps at 5 PL total even with 4+ cards. Mutants may copy Machine Gun or Catapult to gain their "attack non-neighbors" capability.
